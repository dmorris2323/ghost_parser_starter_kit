...
def _collect_week3_files() -> List[Path]:
    """
    Collects canonical Week-3 demo artifacts.
    We include 'latest' plus a couple operator-facing stamped files if present.
    """
    candidates: List[Path] = [
        ...
        # One-pager outputs (written by 7C)
...
def run_week3_cloud_demo_packager(
    ...
    # 1) Generate/update the one-pager first (7C)
    ...
    # In strict mode, we still produce the zip, but mark verdict FAIL if required files missing.
    required = [
    ...
    req_missing = [r for r in required if (ROOT / r).exists() is False]
    ...
    manifest: Dict[str, Any] = {
        ...
        "one_pager_generation": one_pager_out,
        "notes": [
    ...
    # 5) Output envelope (printed to stdout)
